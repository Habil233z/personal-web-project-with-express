...
export async function createProject(req, res, db, dataUri) {
    try {
        ...
        if (img.length === 0) {
        req.flash("error", "Project must have a img")
        res.redirect("/my-project")
        } 
        
        else {
        const query = "INSERT INTO projects (name, description, tag, img, author_id) VALUES ($1, $2, $3, $4, $5) RETURNING*"
        const values = [name, description, tag, img, author_id]
...
export async function deleteProject(req, res, db) {
    ...
    const queryDelete = "DELETE FROM projects WHERE id = $1 AND author_id = $2"
    const resultDelete = await db.query(queryDelete, [id, userId])
